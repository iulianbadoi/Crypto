...
for i = 1:500
    ...
    pl = plot([pnt1(1), pnt2(1)], [pnt1(2) , pnt2(2)] ,'LineWidth',1, 'color', 'red');
    ...
    decFunc = [xi(1)+5 (-constParamLst-xParamLst*(xi(1) + 5)) / yParamLst];
    diff = -decFunc + [xi(1) yi(1)];
    quiver(xi(1), yi(1), diff(1), diff(2), 'g');
    
    
    % Nusprendziam kuriuos ismesti is our feasible regiono
    if length(xi) ~= 0
        if(g1(X) < g2(X))
            [grad_x, grad_y] = grad_g1([xi(1), yi(1)]);
            grad_dir_x = grad_x;
            grad_dir_y = grad_y;
        else
            [grad_x, grad_y] = grad_g1([xi(1), yi(1)]);
        ...
        quiver(xi(1), yi(1), grad_dir_x, grad_dir_y, 'm');
        ...
        while(j <= regSize)
            ...
            diff_x = hull_x(j) / grad_dir_x;
            diff_y = hull_y(j) / grad_dir_y;
        ...
        delete(duxas);
        duxas = plot (hull_final_x,hull_final_y, 'black','LineWidth',3);
    end
    %y = ax + b.
